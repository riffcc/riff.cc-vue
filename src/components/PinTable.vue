<template>
  <v-sheet height="360px" class="d-flex flex-column">
    <p class="text-h6 text-center mb-2">{{ title }}</p>
    <v-divider></v-divider>
    <v-table v-if="pins.length > 0" class="my-auto py-4">
      <tbody>
        <tr v-for="pin in pins" :key="pin.node.id">
          ...
          <td>{{ `${pin.node.piece.contentCid.substring(0,
            4)}...${pin.node.piece.contentCid.substring(pin.node.piece.contentCid.length - 4)}` }}</td>
          <td>
    ...
    <p v-else class="text-center ma-auto">No pins found.</p>
  </v-sheet>
</template>
