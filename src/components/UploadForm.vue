<template>
  ...
  <input placeholder="Nyan Cat Meme" name="name" type="text" class="form-input bg-background-secondary mb-5 h-9 px-1.5"
    v-model="uploadStore.name" />
  <div class="relative mb-4">
    <p class="text-sm mb-1 ml-1">Content CID:</p>
    <p class="text-[0.8rem] text-center mb-2">Upload your files:</p>
    
    <div class="border w-full py-4 mb-2">
      <input type="file" @change="handleFileChange" multiple class="hidden" />
      <v-icon name="hi-folder-add" />
    </div>
    <p class="text-[0.8rem] text-center mb-2">or type a valid CID content:</p>
    <input name="cid" type="text" :disabled="uploadStore.inputHasFiles"
      class="form-input w-full bg-background-secondary mb-5 h-9 px-1.5" v-model="uploadStore.contentCid" />
  ...
  <label class="text-sm mb-1 ml-1">Image Thumbnail CID:</label>
  <div class="relative">
    <input placeholder="bafkreico2zlmcinytg2ri26o4mt73i25ikisd3vth6p4du5gcpek4fsace" name="cid" type="text"
      class="form-input w-full bg-background-secondary mb-5 h-9 px-1.5" v-model="uploadStore.details.thumbnailCid" />
    <p class="ml-1 absolute text-xs text-red-400 -bottom-0.5 text-center w-full"
      v-if="uploadStore.details.thumbnailCid && !uploadStore.thumbnailIsValidCID">
      Please enter a valid CIDv0 or CIDv1 ID.
    </p>
  </div>
  <label class="text-sm mb-1 ml-1">Category:</label>
  <select class="form-select bg-background-secondary mb-5 h-9 px-1.5" v-model="uploadStore.category">
  ...
  <button class="py-1.5 px-3 border border-slate-400 rounded mb-4 disabled:text-slate-400" @click="openAdvanced"
      ...
      <div class="pt-12 pb-8 px-4 flex flex-col justify-between h-full">
        ...
        <div v-if="uploadStore.category === 'Music'" class="grid py-4 h-4/6 overflow-y-auto">
          ...
          <input
            ...
            type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5"
            v-model="uploadStore.details.thumbnailCid"
          /> -->
          ...
          <input placeholder="Values separatted with ','" name="tags" type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.tags" />
          ...
          <input placeholder="" name="music-brainz-id" type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.musicBrainzID" />
          <Label :title="'Artist name(s):'" />
          <input placeholder="Values separatted with ','" name="artist-names" type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.artist" />
          <Label :title="'Album title:'" />
          <input placeholder="" name="album-title" type="text" class="form-input bg-background-secondary mb-4 h-9 px-1.5"
          ...
          <input placeholder="" name="initial-release-year" type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.initialReleaseYear" />
          <Label :title="'Release type:'" />
          <select class="form-select bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.releaseType">
          ...
          <Label :title="'Format:'" />
          <select class="form-select bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.format">
            <option disabled hidden value="undefined">Select a option</option>
          ...
          <Label :title="'Bitrate:'" />
          <input placeholder="" name="bitrate" type="text" class="form-input bg-background-secondary mb-4 h-9 px-1.5"
            v-model="uploadStore.details.bitrate" />
          <Label :title="'Media:'" />
          <select class="form-select bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.media">
            <option disabled hidden value="undefined">Select a option</option>
          ...
          <input placeholder="" name="release-description" type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.releaseDescription" />
        </div>
        <div v-if="uploadStore.category === 'Movies'" class="grid py-4 h-4/6 overflow-y-auto">
          <Label :title="'Image/thumbnail IPFS CID:'" :info-icon="true"
            :content="'A thumbnail for the music, specified as an IPFS CID.'" />
          <input placeholder="QmbWqxBEKC3P8tqsKc98xmWNzrzDtRLMiMPL8wBuTGsMnR" name="image-thumbnail-cid" type="text"
            class="form-input bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.thumbnailCid" />
          <Label :title="'Poster:'" />
          <input placeholder="" name="poster" type="text" class="form-input bg-background-secondary mb-4 h-9 px-1.5"
            v-model="uploadStore.details.poster" />
          <Label :title="'TMDB ID:'" />
          <input placeholder="" name="tmbd-id" type="text" class="form-input bg-background-secondary mb-4 h-9 px-1.5"
            v-model="uploadStore.details.TMDBID" />
          <Label :title="'IMDB ID:'" />
          <input placeholder="" name="imbd-id" type="text" class="form-input bg-background-secondary mb-4 h-9 px-1.5"
            v-model="uploadStore.details.IMDBID" />
          <Label :title="'Type:'" />
          <select class="form-select bg-background-secondary mb-4 h-9 px-1.5" v-model="uploadStore.details.type">
            <option disabled hidden value="undefined">Select a option</option>
